...
simsignal_t Radio::packetErrorRateSignal = cComponent::registerSignal(
        "packetErrorRate");
simsignal_t Radio::bitErrorRateSignal = cComponent::registerSignal(
        "bitErrorRate");
simsignal_t Radio::symbolErrorRateSignal = cComponent::registerSignal(
        "symbolErrorRate");

Radio::Radio() :
        id(nextId++), antenna(nullptr), transmitter(nullptr), receiver(nullptr), medium(
                nullptr), displayCommunicationRange(false), displayInterferenceRange(
                false), upperLayerOut(nullptr), upperLayerIn(nullptr), radioIn(
                nullptr), radioMode(RADIO_MODE_OFF), nextRadioMode(
                RADIO_MODE_OFF), previousRadioMode(RADIO_MODE_OFF), receptionState(
                RECEPTION_STATE_UNDEFINED), transmissionState(
                TRANSMISSION_STATE_UNDEFINED), endTransmissionTimer(nullptr), endReceptionTimer(
                nullptr), endSwitchTimer(nullptr) {
}
...
void Radio::initialize(int stage) {
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        transmitter = check_and_cast<ITransmitter *>(
                getSubmodule("transmitter"));
...
std::ostream& Radio::printToStream(std::ostream& stream, int level) const {
    ...
    if (level >= PRINT_LEVEL_TRACE)
        stream << ", antenna = " << printObjectToString(antenna, level - 1)
                << ", transmitter = "
                << printObjectToString(transmitter, level - 1)
...
void Radio::parseRadioModeSwitchingTimes() {
    ...
    while (tok.hasMoreTokens()) {
        switchingTimes[idx / RADIO_MODE_SWITCHING][idx % RADIO_MODE_SWITCHING] =
                atof(tok.nextToken()) * metric;
        idx++;
    }
...
void Radio::startRadioModeSwitch(RadioMode newRadioMode,
        simtime_t switchingTime) {
    EV_DETAIL << "Starting to change radio mode from "
                     << getRadioModeName(radioMode) << " to "
                     << getRadioModeName(newRadioMode) << ".\n";
...
void Radio::completeRadioModeSwitch(RadioMode newRadioMode) {
    EV_DETAIL << "Radio mode changed from "
                     << getRadioModeName(previousRadioMode) << " to "
                     << getRadioModeName(newRadioMode) << endl;
    if (newRadioMode != IRadio::RADIO_MODE_RECEIVER
            && newRadioMode != IRadio::RADIO_MODE_TRANSCEIVER) {
        endReceptionTimer = nullptr;
    } else if (newRadioMode != IRadio::RADIO_MODE_TRANSMITTER
            && newRadioMode != IRadio::RADIO_MODE_TRANSCEIVER) {
        if (endTransmissionTimer->isScheduled()) {
            EV_WARN << "Aborting ongoing transmissions is not supported"
                           << endl;
...
void Radio::startTransmission(cPacket *macFrame) {
    ...
    const RadioFrame *radioFrame = check_and_cast<const RadioFrame *>(
            medium->transmitPacket(this, macFrame));
    EV_INFO << "Transmission of " << (IRadioFrame *) radioFrame << " as "
                   << radioFrame->getTransmission() << " is started.\n";
...
void Radio::endTransmission() {
    RadioFrame *radioFrame =
            static_cast<RadioFrame *>(endTransmissionTimer->removeControlInfo());
    EV_INFO << "Transmission of " << (IRadioFrame *) radioFrame << " as "
                   << radioFrame->getTransmission() << " is completed.\n";
...
void Radio::startReception(RadioFrame *radioFrame) {
    ...
    const IArrival *arrival = medium->getArrival(this,
            radioFrame->getTransmission());
    cMessage *timer = new cMessage("endReception");
    timer->setControlInfo(radioFrame);
    if (arrival->getStartTime() == simTime()) {
        bool isReceptionAttempted = (radioMode == RADIO_MODE_RECEIVER
                || radioMode == RADIO_MODE_TRANSCEIVER)
                && medium->isReceptionAttempted(this, transmission);
        EV_INFO << "START of Reception\n";
        EV_INFO << "Reception of " << (IRadioFrame *) radioFrame << " as "
                       << transmission << " is "
                       << (isReceptionAttempted ? "attempted" : "ignored")
                       << ".\n";
...
void Radio::endReception(cMessage *message) {
    ...
    RadioFrame *radioFrame =
            static_cast<RadioFrame *>(message->getControlInfo());
    EV_INFO << "Reception of " << (IRadioFrame *) radioFrame << " as "
                   << radioFrame->getTransmission() << " is completed.\n";

    if ((radioMode == RADIO_MODE_RECEIVER || radioMode == RADIO_MODE_TRANSCEIVER)
            && message == endReceptionTimer) {
        cPacket *macFrame = medium->receivePacket(this, radioFrame);
        //macFrame->getCreationTime();
        //macFrame->getArrivalTime();
        //EV_INFO << "macFrame->getCreationTime(): " << macFrame->getCreationTime() << ".\n";
        //EV_INFO << "macFrame->getArrivalTime(): " << macFrame->getArrivalTime() << ".\n";
        //EV_INFO << "macFrame->getDuration(): " << macFrame->getDuration() << ".\n";
        //simtime_t duration = macFrame->getCreationTime() - macFrame->getArrivalTime();
        //EV_INFO << "duration: " << duration << ".\n";
        ...
        ReceptionIndication *indication = check_and_cast<ReceptionIndication *>(macFrame->getControlInfo());

        //indication->setMinRSSI(analogModel->getMinRSSI());
        EV_INFO << " radio indication->getMinRSSI()" << indication->getMinRSSI()
                       << " .\n";
        EV_INFO << " radio indication->getMinSNIR()" << indication->getMinSNIR()
                       << " .\n";
        EV_INFO << " radio indication->getPacketErrorRate()" << indication->getPacketErrorRate() << " .\n";

        // FIXME just a test
        //indication->setPacketErrorRate(1212);
        //indication->setBitErrorRate(34343434);
        //EV_INFO << "indication->getPacketErrorRate() from Radio.cc" << indication->getPacketErrorRate() << " .\n";
        ...
        if (!isNaN(indication->getPacketErrorRate())) {
            ...
            EV_INFO << "minSNIRSignal" << indication->getMinSNIR() << " .\n";
        }
...
bool Radio::isListeningPossible() {
    ...
    const IListening *listening = receiver->createListening(this, now,
            now + 1E-12, position, position);
    const IListeningDecision *listeningDecision = medium->listenOnMedium(this,
            listening);
...
void Radio::updateTransceiverState() {
    ...
    if (radioMode == RADIO_MODE_OFF || radioMode == RADIO_MODE_SLEEP
            || radioMode == RADIO_MODE_TRANSMITTER)
    ...
    if (receptionState != newRadioReceptionState) {
        EV_INFO << "Changing radio reception state from "
                       << getRadioReceptionStateName(receptionState) << " to "
                       << getRadioReceptionStateName(newRadioReceptionState)
                       << ".\n";
        receptionState = newRadioReceptionState;
    ...
    if (transmissionState != newRadioTransmissionState) {
        EV_INFO << "Changing radio transmission state from "
                       << getRadioTransmissionStateName(transmissionState)
                       << " to "
...
void Radio::updateDisplayString() {
    ...
    if (hasGUI() && (displayInterferenceRange || displayCommunicationRange)) {
        ...
        if (displayInterferenceRange) {
            m maxInterferenceRage =
                    check_and_cast<const RadioMedium *>(medium)->getMediumLimitCache()->getMaxInterferenceRange(
                            this);
            char tag[32];
