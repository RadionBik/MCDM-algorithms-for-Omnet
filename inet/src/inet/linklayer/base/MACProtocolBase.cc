...
MACProtocolBase::MACProtocolBase() :
        upperLayerInGateId(-1), upperLayerOutGateId(-1), lowerLayerInGateId(-1), lowerLayerOutGateId(
                -1), interfaceEntry(nullptr) {
}
...
void MACProtocolBase::registerInterface() {
    ...
    IInterfaceTable *interfaceTable = findModuleFromPar<IInterfaceTable>(
...
void MACProtocolBase::sendUp(cMessage *message) {
    EV << " sendUp from macProtocolBase message->getFullName(): "
              << message->getFullName() << endl;
    EV << " sendUp from macProtocolBase message->getFullPath(): "
              << message->getFullPath() << endl;
    if (message->isPacket())
...
void MACProtocolBase::sendDown(cMessage *message) {
    EV << " sendDown from macProtocolBase message->getFullName(): "
              << message->getFullName() << endl;
    EV << " sendDown from macProtocolBase message->getFullPath(): "
              << message->getFullPath() << endl;
    if (message->isPacket())
