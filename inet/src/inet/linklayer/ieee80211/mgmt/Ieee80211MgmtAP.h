...
public:
    /** State of a STA */
    enum STAStatus {
        NOT_AUTHENTICATED, AUTHENTICATED, ASSOCIATED
    };
    ...
    /** Describes a STA inclusive STA apps */
    struct STAInfoDetailed {
        MACAddress address;
        APPList appList;
    };

    typedef std::map<MACAddress, STAInfo, MAC_compare> STAList;
    typedef std::map<MACAddress, STAInfoDetailed, MAC_compare> STAConfiguredList;
...
protected:
    ...
    STAConfiguredList staConfiguredList;    ///< list of all configured STAs
    std::string apName;
    std::map <std::string, double> apCurrentLoadMap;
    double loadBalacingCoeffPrev = 0;

    // for statistics
    static simsignal_t availBitrateSignal;
    static simsignal_t loadBalCoeffSignal;

    static simsignal_t currentLoadSignal;
    cMessage *availBitrateCollectionTimer = nullptr;
    cMessage *loadBalanceIndexCalcTimer = nullptr;

    std::vector <double> currentLoadApBuff; // just a buffer for the loadAp values from the apCurrentLoadMap


    double bitrate = 0;  // configured bitrate
    double configuredThroughputAssocSTAs = 0;
    double availBitrate = 0; // calculated by bitrate - configuredThroughputAssocSTAs
    double currentRelativeLoad = 0; // calculated by configuredThroughputAssocSTAs/bitrate
...
protected:
    ...
    virtual void sendManagementFrame(Ieee80211ManagementFrame *frame,
    ...
    virtual void handleAuthenticationFrame(Ieee80211AuthenticationFrame *frame)
            override;
    virtual void handleDeauthenticationFrame(
            Ieee80211DeauthenticationFrame *frame) override;
    virtual void handleAssociationRequestFrame(
            Ieee80211AssociationRequestFrame *frame) override;
    virtual void handleAssociationResponseFrame(
            Ieee80211AssociationResponseFrame *frame) override;
    virtual void handleReassociationRequestFrame(
            Ieee80211ReassociationRequestFrame *frame) override;
    virtual void handleReassociationResponseFrame(
            Ieee80211ReassociationResponseFrame *frame) override;
    ...
    //@}
    // EEW
    double getThroughputGeneratedByNode(const MACAddress& _macAddress);
    void getWirelessHostsAndTheirApps();
    double getConfiguredThroughputOfAccociatedSTAs();
    void updateStaList(std::string _staAddressDisassoc);
