...
Register_Enum(inet::Ieee80211Mac,
        (Ieee80211Mac::IDLE, Ieee80211Mac::DEFER, Ieee80211Mac::WAITAIFS, Ieee80211Mac::BACKOFF, Ieee80211Mac::WAITACK, Ieee80211Mac::WAITMULTICAST, Ieee80211Mac::WAITCTS, Ieee80211Mac::WAITSIFS, Ieee80211Mac::RECEIVE));

/****************************************************************
 * Construction functions.
 */

simsignal_t Ieee80211Mac::beaconDelaySignal = registerSignal("beaconDelay");
simsignal_t Ieee80211Mac::beaconJitterSignal = registerSignal("beaconJitter");
...
Ieee80211Mac::~Ieee80211Mac() {
    ...
    for (unsigned int i = 0; i < edcCAF.size(); i++) {
        ...
        while (!transmissionQueue(i)->empty()) {
            Ieee80211Frame *temp =
                    dynamic_cast<Ieee80211Frame *>(transmissionQueue(i)->front());
...
void Ieee80211Mac::initialize(int stage) {
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        if (par("EDCA")) {
            ...
            classifier = check_and_cast<IQoSClassifier *>(
                    inet::utils::createOne(classifierClass));
        ...
        if (classifier && dynamic_cast<Ieee80211eClassifier *>(classifier))
            static_cast<Ieee80211eClassifier *>(classifier)->setDefaultClass(
                    defaultAC);
        ...
        for (int i = 0; i < numCategories(); i++) {
            ...
            if (hasPar(strAifs.c_str()) && hasPar(strTxop.c_str())) {
                ...
                throw cRuntimeError("parameters %s , %s don't exist",
                        strAifs.c_str(), strTxop.c_str());
        }
        if (numCategories() == 1)
            AIFSN(0) = par("AIFSN");
        ...
        EV_DEBUG << " slotTime=" << getSlotTime() * 1e6 << "us DIFS="
                        << getDIFS() * 1e6 << "us";
        ...
        EV_DEBUG << " bitrate=" << bitrate / 1e6 << "Mb IDLE=" << IDLE
        ...
        EV << "throughputTimePeriod:" << throughputTimePeriod << endl;
        ...
        cModule *radioModule =
                gate("lowerLayerOut")->getNextGate()->getOwnerModule();
...
void Ieee80211Mac::handleSelfMessage(cMessage *msg) {
    if (msg == throughputTimer) {
        throughputLastPeriod = recBytesOverPeriod
                / SIMTIME_DBL(throughputTimePeriod);
        recBytesOverPeriod = 0;
        scheduleAt(simTime() + throughputTimePeriod, throughputTimer);
        return;
    }

    EV_DEBUG << "received self message: " << msg << "(kind: " << msg->getKind()
                    << ")" << endl;
    ...
    if ((strcmp(msg->getName(), "Backoff") == 0)
            || (strcmp(msg->getName(), "AIFS") == 0)) {
        int kind;
        kind = msg->getKind();
        if (kind < 0)
            kind = 0;
        EV_DEBUG << " kind is " << kind << ",name is " << msg->getName()
                        << endl;
        for (unsigned int i = numCategories() - 1; (int) i > kind; i--) { //mozna prochaze jen 3..kind XXX
            if (((endBackoff(i)->isScheduled()
                    && endBackoff(i)->getArrivalTime() == simTime())
                    || (endAIFS(i)->isScheduled() && !backoff(i)
                            && endAIFS(i)->getArrivalTime() == simTime()))
                    && !transmissionQueue(i)->empty()) {
                EV_DEBUG << "Internal collision AC" << kind << " with AC" << i
                                << endl;
                numInternalCollision++;
                EV_DEBUG << "Cancel backoff event and schedule new one for AC"
                                << kind << endl;
                cancelEvent(endBackoff(kind));
                if (retryCounter() == transmissionLimit - 1) {
                    EV_WARN << "give up transmission for AC" << currentAC
                                   << endl;
...
void Ieee80211Mac::handleUpperPacket(cPacket *msg) {
    if (queueModule && numCategories() > 1
            && (int) transmissionQueueSize() < maxQueueSize) {
    ...
    //EV_DEBUG << "this->throughputTotalAP: " << this->throughputTotalAP << endl;
    ...
    Ieee80211DataOrMgmtFrame *frame =
            check_and_cast<Ieee80211DataOrMgmtFrame *>(msg);
    if (frame->getByteLength() > fragmentationThreshold)
        throw cRuntimeError(
                "message from higher layer (%s)%s is too long for 802.11b, %d bytes (fragmentation is not supported yet)",
                msg->getClassName(), msg->getName(),
                (int) (msg->getByteLength()));
    EV_DEBUG << "frame " << frame << " received from higher layer, receiver = "
...
int Ieee80211Mac::mappingAccessCategory(Ieee80211DataOrMgmtFrame *frame) {
    ...
    if (isDataFrame && maxQueueSize
            && (int) transmissionQueueSize() >= maxQueueSize) {
        EV_WARN << "message " << frame
                       << " received from higher layer but AC queue is full, dropping message\n";
        numDropped()++;delete
frame        ;
        return 200;
    }
    if (isDataFrame) {
        if (!prioritizeMulticast || !frame->getReceiverAddress().isMulticast()
                || transmissionQueue()->size() < 2)
        ...
        if (transmissionQueue()->size() < 2) {
            ...
            p++;
            while ((p != transmissionQueue()->end())
                    && (dynamic_cast<Ieee80211DataFrame *>(*p) == nullptr)) // search the first not management frame
                p++;
            transmissionQueue()->insert(p, frame);
        }
    }
    EV_DEBUG << "frame classified as access category " << currentAC
...
void Ieee80211Mac::handleUpperCommand(cMessage *msg) {
    if (msg->getKind() == RADIO_C_CONFIGURE) {
        EV_DEBUG << "Passing on command " << msg->getName()
                        << " to physical layer\n";
        if (pendingRadioConfigMsg != nullptr) {
            // merge contents of the old command into the new one, then delete it
            Ieee80211ConfigureRadioCommand *oldConfigureCommand =
                    check_and_cast<Ieee80211ConfigureRadioCommand *>(
                            pendingRadioConfigMsg->getControlInfo());
            Ieee80211ConfigureRadioCommand *newConfigureCommand =
                    check_and_cast<Ieee80211ConfigureRadioCommand *>(
                            msg->getControlInfo());
            if (newConfigureCommand->getChannelNumber() == -1
                    && oldConfigureCommand->getChannelNumber() != -1)
                newConfigureCommand->setChannelNumber(
                        oldConfigureCommand->getChannelNumber());
        ...
        if (fsm.getState() == IDLE || fsm.getState() == DEFER
                || fsm.getState() == BACKOFF) {
            ...
            EV_DEBUG << "Delaying " << msg->getName()
                            << " until next IDLE or DEFER state\n";
            pendingRadioConfigMsg = msg;
        }
    } else {
        throw cRuntimeError(
                "Unrecognized command from mgmt layer: (%s)%s msgkind=%d",
...
void Ieee80211Mac::handleLowerPacket(cPacket *msg) {
    // EEW
    Ieee80211DataOrMgmtFrame *mframe =
            dynamic_cast<Ieee80211DataOrMgmtFrame *>(msg);
    if (mframe) {
        EV_INFO << "this is a frame from mac" << ".\n";
        if (mframe->getType() == ST_BEACON) {

            // set data into controlInfo
            ReceptionIndication *ctl =
                    dynamic_cast<ReceptionIndication *>(mframe->getControlInfo());
            if (ctl != nullptr) {
                EV_DEBUG << "controlInfo accessed" << endl;
                //ctl->setBitErrorRate(121212);
            }
        }
    }

    EV_TRACE << "->Enter handleLowerMsg...\n";
    EV_DEBUG << " 050505 received message from lower layer: " << msg << endl;
    EV_DEBUG << " 050505 received message from lower layer: "
                    << this->getFullName() << endl;
    EV_DEBUG << " 050505 received message from lower layer: "
                    << this->getFullPath() << endl;
    Ieee80211ReceptionIndication *cinfo =
            dynamic_cast<Ieee80211ReceptionIndication *>(msg->getControlInfo());
    if (cinfo && cinfo->getAirtimeMetric()) {
        double rtsTime = 0;
        if (rtsThreshold * 8 < cinfo->getTestFrameSize())
            rtsTime = controlFrameTxTime(LENGTH_CTS)
                    + controlFrameTxTime(LENGTH_RTS);
    ...
    if (msg->getControlInfo()
            && dynamic_cast<Ieee80211ReceptionIndication *>(msg->getControlInfo())) {
        Ieee80211ReceptionIndication *cinfo =
                (Ieee80211ReceptionIndication *) msg->removeControlInfo();
        if (contJ % 10 == 0) {
            snr = _snr;
            contJ = 0;
            _snr = 0;
    ...
    if (contI % samplingCoeff == 0) {
        contI = 0;
        recvdThroughput = 0;
        EV_DEBUG << " 112233 recvdThroughput set to zero" << endl;
    }
    contI++;
    ...
    if (timeStampLastMessageReceived == SIMTIME_ZERO)
        ...
        if (frame)
            recvdThroughput += ((frame->getBitLength()
                    / (simTime() - timeStampLastMessageReceived)) / 1000000)
                    / samplingCoeff;
        timeStampLastMessageReceived = simTime();
    }
    ...
    if (!frame) {
        EV_ERROR
                        << "message from physical layer (%s)%s is not a subclass of Ieee80211Frame"
    ...
    EV_DEBUG << " 112233 Self address: " << address << ", receiver address: "
                    << frame->getReceiverAddress()
                    << ", received frame is for us: " << isForUs(frame)
                    << ", received frame was sent by us: " << isSentByUs(frame)
                    //<< ", sender address: " << frame->getse
                    << endl;
    EV_DEBUG << " 112233 recvdThroughput: " << recvdThroughput << endl;

    Ieee80211TwoAddressFrame *twoAddressFrame =
            dynamic_cast<Ieee80211TwoAddressFrame *>(msg);
    ASSERT(
            !twoAddressFrame
                    || twoAddressFrame->getTransmitterAddress() != address);

    // EEW
    // calculate delay here
    simtime_t delayAP;
    EV_INFO << "555 simTime() " << simTime() << ".\n";
    EV_INFO << "555 frame->getMACArrive() " << frame->getMACArrive() << ".\n";
    EV_INFO << "555 frame->getCreationTime(): " << frame->getCreationTime()
                   << ".\n";
    EV_INFO << "555 frame->getArrivalTime(): " << frame->getArrivalTime()
                   << ".\n";
    beaconDelay = frame->getArrivalTime() - frame->getCreationTime();

    EV_DEBUG << "555 calc beaconDelay ...: " << beaconDelay << endl;

    // EEW put the delay value into the mac frame
    Ieee80211DataOrMgmtFrame *aframe =
            dynamic_cast<Ieee80211DataOrMgmtFrame *>(msg);
    if (aframe) {
        EV_INFO << "this is a frame from mac" << ".\n";
        if (aframe->getType() == ST_BEACON) {

            // set data into controlInfo
            ReceptionIndication *ctl =
                    dynamic_cast<ReceptionIndication *>(aframe->getControlInfo());
            if (ctl != nullptr) {
                EV_DEBUG << "controlInfo accessed" << endl;
                ctl->setBitErrorRate(121212);
            }
            ...
            // calculate delay here
            /*
             EV_INFO << "5555 simTime() " << simTime() << ".\n";
             EV_INFO << "5555 frame->getMACArrive() " << frame->getMACArrive() << ".\n";
             EV_INFO << "5555 frame->getCreationTime(): " << frame->getCreationTime()
             << ".\n";
             EV_INFO << "5555 frame->getArrivalTime(): " << frame->getArrivalTime()
             << ".\n";
             delayAP = frame->getArrivalTime() - frame->getCreationTime();*/

            EV_DEBUG << "5555 calc beaconDelay ...: " << beaconDelay << endl;

            Ieee80211BeaconFrame *bframe =
                    dynamic_cast<Ieee80211BeaconFrame *>(msg);

            EV_INFO << " 777 --- " << simTime() << ".\n";
            EV_INFO << " 777 simtime() " << simTime() << ".\n";
            EV_INFO << " 777 bframe->getCreationTime() "
                           << bframe->getCreationTime() << ".\n";
            EV_INFO << " 777 bframe->getArrivalTime() "
                           << bframe->getArrivalTime() << ".\n";
            EV_INFO << " 777 aframe->getMACArrive() " << bframe->getMACArrive()
                           << ".\n";
            EV_INFO
                           << " 777 aframe->getArrivalTime() - bframe->getCreationTime()"
                           << bframe->getArrivalTime()
                                   - bframe->getCreationTime() << ".\n";
            EV_INFO << " 777 simTime() - aframe->getMACArrive() "
                           << simTime() - bframe->getMACArrive() << ".\n";
            //delayAP = simTime() - bframe->getMACArrive();
            //EV_INFO << " 777 delayAP " << delayAP << ".\n";
            //delayAP = bframe->getArrivalTime() - bframe->getCreationTime();
            //EV_INFO << " 777 delayAP 2 " << delayAP << ".\n";
...
void Ieee80211Mac::receiveSignal(cComponent *source, simsignal_t signalID,
        long value) {
    Enter_Method_Silent
    ();
...
void Ieee80211Mac::handleWithFSM(cMessage *msg) {
    ...
    // EEW
    double throughputTotalAP = 0;
    simtime_t delayAP = simTime();
    simtime_t jitterAP = 0;
    double avrBeaconDelay = 0;
    double avrBeaconJitter = 0;

    EV_DEBUG << "121212 start of handleWithFSM \n",
    ...
    if (isUpperMessage(msg) && fsm.getState() != IDLE) {
        ...
        EV_DEBUG << "deferring upper message transmission in "
                        << fsm.getStateName() << " state\n";
    ...
    if (msg == endTimeout
            && radio->getReceptionState() == IRadio::RECEPTION_STATE_RECEIVING
            && useModulationParameters && fsm.getState() == WAITACK) {
    ...
    // get delay only for received beacons
    /*Ieee80211DataOrMgmtFrame *mframe =
     dynamic_cast<Ieee80211DataOrMgmtFrame *>(msg);
     if (mframe) {
     EV_INFO << "this is a frame from mac" << ".\n";
     ...
     EV_INFO << " 999 --- " << simTime() << ".\n";
     EV_INFO << " 999 simtime() " << simTime() << ".\n";
     fr = getCurrentTransmission();
     //EV_INFO << " 999 fr->getMACArrive() " << fr->getMACArrive() << ".\n";
     EV_INFO << " 999 bframe->getCreationTime() "
     << mframe->getCreationTime() << ".\n";
     EV_INFO << " 999 bframe->getArrivalTime() "
     << mframe->getArrivalTime() << ".\n";
     EV_INFO << " 999 aframe->getMACArrive() " << mframe->getMACArrive()
     << ".\n";
     EV_INFO
     << " 999 aframe->getArrivalTime() - bframe->getCreationTime()"
     << mframe->getArrivalTime()
     - mframe->getCreationTime() << ".\n";
     EV_INFO << " 999 simTime() - aframe->getMACArrive() "
     << simTime() - mframe->getMACArrive() << ".\n";
     delayAP = simTime() - mframe->getMACArrive();
     EV_INFO << " 999 delayAP " << delayAP << ".\n";
     delayAP = mframe->getArrivalTime() - mframe->getCreationTime();
     EV_INFO << " 999 delayAP 2 " << delayAP << ".\n";
    ...
    if (frame && isLowerMessage(frame)) {
        lastReceiveFailed = receptionError =
                frame ? frame->hasBitError() : false;
    ...
    FSMA_State(WAITACK) {
        ...
        FSMA_Event_Transition(Receive - ACK - TXOP - Empty,
                ...
                macDelayRecorded()=simTime() - fr->getMACArrive();
                EV_DEBUG << "record  fr->getMACArrive()" << fr->getMACArrive() << endl;
                delayAP = simTime() - fr->getMACArrive();
                ...
                EV_DEBUG << "record delayAP" << delayAP << endl;
                numSentTXOP++;
                cancelTimeoutPeriod();
    ...
    FSMA_State(RECEIVE) {
        ...
        FSMA_No_Event_Transition(Immediate - Receive - Multicast,
                ...
                IDLE,
                sendUp(frame);
                EV << "sendUp frame " << frame->getName() << endl;
    ...
    EV_DEBUG << " 009 *** record data in ...Mac.cc " << endl;
    EV_DEBUG << " 009 record data numCategories()" << numCategories() << endl;
    EV_DEBUG << " 009 record data simTime()" << simTime() << endl;
    EV_DEBUG << " record data last" << last << endl;
    stateVector.record(fsm.getState());

    if (simTime() - last > 0.1) {
        for (int i = 0; i < numCategories(); i++) {
            throughput(i)->record(bits(i) / (simTime() - last));
            throughputRecorded(i) = bits(i) / (simTime() - last);

            // check if it is up or down message
            if (isUpperMessage(msg)) {
                EV_DEBUG << " 808080 --- upper message " << endl;
            } else {
                EV_DEBUG << " 808080 --- lower message " << endl;
            }

            /*EV_DEBUG << " 808080 record throughput " << simTime() << " "
             << aframe->getTransmitterAddress() << " "
             << aframe->getReceiverAddress() << " "
             << bits(i) / (simTime() - last) << endl; */
            //EV_DEBUG << " record throughputTotalAP " << throughputTotalAP
            //                << endl;
            EV_DEBUG << "macDelayRecorded() " << macDelayRecorded(i) << endl;

            delayAP = macDelayRecorded(i);
            bits(i) = 0;
            if (maxJitter(i) > SIMTIME_ZERO && minJitter(i) > SIMTIME_ZERO) {
                jitter(i)->record(maxJitter(i) - minJitter(i));

                jitterRecorded(i) = maxJitter(i) - minJitter(i);
                EV_DEBUG << "minJitter(i)" << minJitter(i) << endl;
                EV_DEBUG << "maxJitter(i)" << maxJitter(i) << endl;
                EV_DEBUG << "jitterRecorded()" << jitterRecorded(i) << endl;
                maxJitter(i) = SIMTIME_ZERO;
                minJitter(i) = SIMTIME_ZERO;
            }
        }
    ...
    // EEW put the throughput into control info of the mac frame
    Ieee80211DataOrMgmtFrame *aframe =
            dynamic_cast<Ieee80211DataOrMgmtFrame *>(msg);
    if (aframe) {
        //EV_INFO << " 9999 this is a frame from mac" << ".\n";
        if (aframe->getType() == ST_BEACON) {

            //delayAP = aframe->getArrivalTime() - aframe->getCreationTime();

            Ieee80211BeaconFrame *bframe =
                    dynamic_cast<Ieee80211BeaconFrame *>(msg);
            if (bframe) {

                if (previousBeaconDelay != 0) {
                    if (beaconDelay > previousBeaconDelay) {
                        beaconJitter = beaconDelay - previousBeaconDelay;
                    } else {
                        beaconJitter = previousBeaconDelay - beaconDelay;
                    }
                } else {
                    beaconJitter = 0;
                }

                EV_INFO << " 99999999 --- simTime()" << simTime() << ".\n";
                EV_INFO << " 9999 this is a beacon frame from mac" << ".\n";
                EV_DEBUG << "9999 calc previousBeaconDelay ...: "
                                << previousBeaconDelay << endl;
                EV_DEBUG << "9999 calc beaconDelay ...: " << beaconDelay
                                << endl;
                EV_DEBUG << "9999 calc beaconJitter ...: " << beaconJitter
                                << endl;
                emit(beaconDelaySignal, beaconDelay); // macDelay measured on beacons
                emit(beaconJitterSignal, beaconJitter); // macDelay measured on beacons
                previousBeaconDelay = beaconDelay;

                beaconDelayList.push_back(beaconDelay);

                avrBeaconDelay = getAvrBeaconDelay(beaconDelayList);
                EV_DEBUG << "9999 avrBeaconDelay: " << avrBeaconDelay << endl;

                beaconJitterList.push_back(beaconJitter);
                avrBeaconJitter = getAvrBeaconJitter(beaconJitterList);
                                EV_DEBUG << "9999 avrBeaconJitter: " << avrBeaconJitter << endl;

                if (avrBeaconDelay==0){
                    EV_DEBUG << "999921 " << aframe->getTransmitterAddress() << endl;
                }


                for (int i = 0; i < numCategories(); i++) {
                    // set the throughput only for the self messages
                    // in this way the total throughput on the AP is collected
                    if (aframe->getTransmitterAddress() == address) {
                        this->throughputTotalAP = throughputRecorded(i);
                    }

                    EV_DEBUG << " 9999 ---throughputRecorded(i) "
                                    << throughputRecorded(i) << endl;
                    throughputTotalAP = throughputRecorded(i);
                    //EV_DEBUG << " 9999 ---macDelayRecorded() "
                    //                << macDelayRecorded(i) << endl;
                    delayAP = macDelayRecorded(i);

                    //EV_DEBUG << " 9999 ---jitterRecorded(i) "
                    //                << jitterRecorded(i) << endl;
                    //jitterAP = jitterRecorded(i);
                }

                EV_DEBUG << " 9999 bframe->getTransmitterAddress() "
                                << bframe->getTransmitterAddress() << endl;
                EV_DEBUG << " 9999 address " << address << endl;
                EV_DEBUG << " 9999 throughputTotalAP of all flows on the AP "
                                << this->throughputTotalAP << endl;
                EV_DEBUG << "put into body throughputTotalAP "
                                << throughputTotalAP << endl;
                //EV_DEBUG << "put into body jitterAP" << jitterAP << endl;

                // if it is an beacon from upper layer, set the the throughputTotalAP parameter,
                // to be send to the client
                if (isUpperMessage(msg)) {
                    bframe->getBody().getSupportedRates().rate[3] =
                            throughputTotalAP;
                    EV_DEBUG << " 007007 AP throughput: " << throughputTotalAP
                                    << endl;
                } else {
                    // if not put the delay and jitter for the upper layers
                    //bframe->getBody().getSupportedRates().rate[0] =
                    //        avrBeaconDelay;
                    bframe->getBody().getSupportedRates().rate[0]=avrBeaconDelay;

                    bframe->getBody().getSupportedRates().rate[1] =
                            avrBeaconJitter;
                    //bframe->getBody().getSupportedRates().rate[1] =
                    //                            jitterAP.dbl();
                    // this is the local throughput of STA
                    bframe->getBody().getSupportedRates().rate[6] =
                            throughputTotalAP;
                    EV_DEBUG << " 007007 STA throughput: " << throughputTotalAP
                                    << endl;
                }

                //bframe->getBody().getSupportedRates().rate[1] = 111;
                //bframe->getBody().getSupportedRates().rate[2] = 222;
                //bframe->getBody().getSupportedRates().rate[3] =
                //        throughputTotalAP;

                /*
                 EV_DEBUG << "get from body throughputTotalAP "
                 << bframe->getBody().getThroughputTotalAP()
                 << endl;
                 EV_DEBUG << "get from body delayAP"
                 << bframe->getBody().getDelay() << endl;
                 EV_DEBUG << "get from body jitterAP"
                 << bframe->getBody().getJitter() << endl; */

                // put the data into controlInfo here
                //ReceptionIndication *indication = check_and_cast<ReceptionIndication *>(aframe->getControlInfo());
                //                indication->setPacketErrorRate(3434);
                /*ReceptionIndication *ctl =
                 dynamic_cast<ReceptionIndication *>(aframe->getControlInfo());
                 if (ctl != nullptr) {
                 EV_DEBUG << "controlInfo accessedddddd" << endl;
                 ctl->setBitErrorRate(12121212121212);
                 }*/
...
double Ieee80211Mac::getAvrBeaconDelay(std::list<simtime_t> _beaconDelayList) {
    double avrBeaconDelay = 0;
    double sumBeaconDelay = 0;
    int overSize = 0;

    // keep only last 10 entries
    if (_beaconDelayList.size() > 10) {
        overSize = _beaconDelayList.size() - 10;
        for (int i = 0; i < overSize; i++) {
            _beaconDelayList.pop_front();
        }
    }

    for (std::list<simtime_t>::iterator it = _beaconDelayList.begin();
            it != _beaconDelayList.end(); ++it) {
        std::cout << ' ' << *it;
        sumBeaconDelay = sumBeaconDelay + (*it).dbl();
        //EV_DEBUG << " 9999      beaconDelay: " << (*it).dbl() << endl;
        //EV_DEBUG << " 9999          sumBeaconDelay: " << sumBeaconDelay << endl;
    }

    avrBeaconDelay = sumBeaconDelay / _beaconDelayList.size();
    //EV_DEBUG << " 9999 _beaconDelayList.size(): " << _beaconDelayList.size()
    //                << endl;
    //EV_DEBUG << " 9999 sumBeaconDelay: " << sumBeaconDelay << endl;
    //EV_DEBUG << " 9999 avrBeaconDelay: " << avrBeaconDelay << endl;
    return avrBeaconDelay;
}


double Ieee80211Mac::getAvrBeaconJitter(std::list<simtime_t> _beaconJitterList) {
    double avrBeaconJitter = 0;
    double sumBeaconJitter = 0;
    int overSize = 0;

    // keep only last 10 entries
    if (_beaconJitterList.size() > 10) {
        overSize = _beaconJitterList.size() - 10;
        for (int i = 0; i < overSize; i++) {
            _beaconJitterList.pop_front();
        }
    }

    for (std::list<simtime_t>::iterator it = _beaconJitterList.begin();
            it != _beaconJitterList.end(); ++it) {
        std::cout << ' ' << *it;
        sumBeaconJitter = sumBeaconJitter + (*it).dbl();
        EV_DEBUG << " 9999      beaconJitter: " << (*it).dbl() << endl;
        EV_DEBUG << " 9999          sumBeaconJitter: " << sumBeaconJitter << endl;
    }

    avrBeaconJitter = sumBeaconJitter / _beaconJitterList.size();
    EV_DEBUG << " 9999 _beaconJitterList.size(): " << _beaconJitterList.size()
                    << endl;
    EV_DEBUG << " 9999 sumBeaconJitter: " << sumBeaconJitter << endl;
    EV_DEBUG << " 9999 avrBeaconJitter: " << avrBeaconJitter << endl;
    return avrBeaconJitter;
}
...
simtime_t Ieee80211Mac::computeBackoffPeriod(Ieee80211Frame *msg, int r) {
    ...
    EV_DEBUG << "generated backoff slot number: " << c << " , cw: " << cw
                    << " ,cwMin:cwMax = " << cwMin() << ":" << cwMax() << endl;
...
void Ieee80211Mac::scheduleAIFSPeriod() {
    ...
    for (int i = 0; i < numCategories(); i++) {
        if (!endAIFS(i)->isScheduled() && !transmissionQueue(i)->empty()) {
            if (lastReceiveFailed) {
                EV_DEBUG
                                << "reception of last frame failed, scheduling EIFS-DIFS+AIFS period ("
                                << i << ")\n";
                scheduleAt(simTime() + getEIFS() - getDIFS() + getAIFS(i),
                        endAIFS(i));
            } else {
...
void Ieee80211Mac::scheduleDataTimeoutPeriod(
        Ieee80211DataOrMgmtFrame *frameToSend) {
    double tim;
    double bitRate = dataFrameMode->getDataMode()->getNetBitrate().get();
    TransmissionRequest *trq =
            dynamic_cast<TransmissionRequest *>(frameToSend->getControlInfo());
    if (trq) {
    ...
    if (!endTimeout->isScheduled()) {
        ...
        if (useModulationParameters) {
            ...
            tim = computeFrameDuration(frameToSend) + SIMTIME_DBL(getSlotTime())
                    + SIMTIME_DBL(getSIFS()) + controlFrameTxTime(LENGTH_ACK)
...
void Ieee80211Mac::scheduleMulticastTimeoutPeriod(
        Ieee80211DataOrMgmtFrame *frameToSend) {
    if (!endTimeout->isScheduled()) {
...
void Ieee80211Mac::scheduleCTSTimeoutPeriod() {
    if (!endTimeout->isScheduled()) {
        ...
        scheduleAt(
                simTime() + controlFrameTxTime(LENGTH_RTS) + getSIFS()
                        + controlFrameTxTime(LENGTH_CTS)
                        + MAX_PROPAGATION_DELAY * 2, endTimeout);
    }
}
...
void Ieee80211Mac::generateBackoffPeriod() {
    backoffPeriod() = computeBackoffPeriod(getCurrentTransmission(),
            retryCounter());
...
void Ieee80211Mac::decreaseBackoffPeriod() {
    ...
    for (int i = 0; i < numCategories(); i++) {
        if (backoff(i) && endBackoff(i)->isScheduled()) {
            EV_DEBUG << "old backoff[" << i << "] is " << backoffPeriod(i)
                            << ", sim time is " << simTime()
                            << ", endbackoff sending period is "
                            << endBackoff(i)->getSendingTime() << endl;
            simtime_t elapsedBackoffTime = simTime()
                    - endBackoff(i)->getSendingTime();
            backoffPeriod(i) -= ((int) (elapsedBackoffTime / getSlotTime()))
                    * getSlotTime();
            EV_DEBUG << "actual backoff[" << i << "] is " << backoffPeriod(i)
                            << ", elapsed is " << elapsedBackoffTime << endl;
            ASSERT(backoffPeriod(i) >= SIMTIME_ZERO);
            EV_DEBUG << "backoff[" << i << "] period decreased to "
                            << backoffPeriod(i) << endl;
        }
    }
}
...
void Ieee80211Mac::sendACKFrameOnEndSIFS() {
    Ieee80211Frame *frameToACK =
            (Ieee80211Frame *) endSIFS->getContextPointer();
    endSIFS->setContextPointer(nullptr);
...
void Ieee80211Mac::sendDataFrameOnEndSIFS(
        Ieee80211DataOrMgmtFrame *frameToSend) {
...
void Ieee80211Mac::sendDataFrame(Ieee80211DataOrMgmtFrame *frameToSend) {
    ...
    if (!txop && TXOP() > 0 && transmissionQueue()->size() >= 2) {
        ...
        for (frame = transmissionQueue()->begin();
                frame != transmissionQueue()->end(); ++frame) {
            count++;
        ...
        EV_DEBUG << "scheduling TXOP for AC" << currentAC << ", duration is "
                        << time << ", count is " << count << endl;
    ...
    sendDown(
            buildDataFrame(
                    dynamic_cast<Ieee80211DataOrMgmtFrame *>(setBitrateFrame(
                            frameToSend))));
}
...
void Ieee80211Mac::sendMulticastFrame(Ieee80211DataOrMgmtFrame *frameToSend) {
    ...
    sendDown(
            buildDataFrame(
                    dynamic_cast<Ieee80211DataOrMgmtFrame *>(setBasicBitrate(
                            frameToSend))));
}
 ...
 */
Ieee80211DataOrMgmtFrame * Ieee80211Mac::buildDataFrame(
        Ieee80211DataOrMgmtFrame * frameToSend) {
    Ieee80211DataOrMgmtFrame *frame =
            (Ieee80211DataOrMgmtFrame *) frameToSend->dup();
    ...
    if (frameToSend->getControlInfo() != nullptr) {
        ...
        if (ctrl == nullptr)
            throw cRuntimeError(
                    "control info is not TransmissionRequest but %s",
                    ctr->getClassName());
    ...
    if (isMulticast(frameToSend))
        ...
        if (txop && transmissionQueue()->size() > 1) {
            ...
            double bitRate =
                    dataFrameMode->getDataMode()->getNetBitrate().get();
            int size = (*nextframeToSend)->getBitLength();
            TransmissionRequest *trRq =
                    dynamic_cast<TransmissionRequest *>(transmissionQueue()->front()->getControlInfo());
            if (trRq) {
            ...
            frame->setDuration(
                    3 * getSIFS() + 2 * controlFrameTxTime(LENGTH_ACK)
...
Ieee80211ACKFrame * Ieee80211Mac::buildACKFrame(
        Ieee80211DataOrMgmtFrame * frameToACK) {
    ...
    if (!frameToACK->getMoreFragments())
        ...
        frame->setDuration(
                frameToACK->getDuration() - getSIFS()
                        - controlFrameTxTime(LENGTH_ACK));

    return frame;
}

Ieee80211RTSFrame * Ieee80211Mac::buildRTSFrame(
        Ieee80211DataOrMgmtFrame * frameToSend) {
    Ieee80211RTSFrame *frame = new Ieee80211RTSFrame("wlan-rts");
    frame->setTransmitterAddress(address);
    frame->setReceiverAddress(frameToSend->getReceiverAddress());
    frame->setDuration(
            3 * getSIFS() + controlFrameTxTime(LENGTH_CTS)
                    + computeFrameDuration(frameToSend)
                    + controlFrameTxTime(LENGTH_ACK));
...
Ieee80211CTSFrame * Ieee80211Mac::buildCTSFrame(Ieee80211RTSFrame * rtsFrame) {
    ...
    frame->setDuration(
            rtsFrame->getDuration() - getSIFS()
                    - controlFrameTxTime(LENGTH_CTS));

    return frame;
}

Ieee80211DataOrMgmtFrame * Ieee80211Mac::buildMulticastFrame(
        Ieee80211DataOrMgmtFrame * frameToSend) {
    Ieee80211DataOrMgmtFrame *frame =
            (Ieee80211DataOrMgmtFrame *) frameToSend->dup();

    TransmissionRequest *oldTransmissionRequest =
            dynamic_cast<TransmissionRequest *>(frameToSend->getControlInfo());
...
void Ieee80211Mac::giveUpCurrentTransmission() {
    Ieee80211DataOrMgmtFrame *temp =
            (Ieee80211DataOrMgmtFrame *) transmissionQueue()->front();
...
Ieee80211DataOrMgmtFrame * Ieee80211Mac::getCurrentTransmission() {
    return transmissionQueue()->empty() ?
            nullptr : (Ieee80211DataOrMgmtFrame *) transmissionQueue()->front();
}
...
bool Ieee80211Mac::isMediumFree() {
    return !endReserve->isScheduled()
            && radio->getReceptionState() == IRadio::RECEPTION_STATE_IDLE;
}
...
void Ieee80211Mac::popTransmissionQueue() {
    ...
    Ieee80211Frame *temp =
            dynamic_cast<Ieee80211Frame *>(transmissionQueue()->front());
    ...
    if (queueModule) {
        if (numCategories() == 1) {
            ...
            queueModule->requestPacket();
                ...
                && (int) transmissionQueueSize() == maxQueueSize - 1) {
...
double Ieee80211Mac::computeFrameDuration(Ieee80211Frame *msg) {
    ...
    if (ctrl) {
        EV_DEBUG << "Per frame2 params bitrate "
                        << ctrl->getBitrate().get() / 1e6 << "Mb" << endl;
        duration = computeFrameDuration(msg->getBitLength(),
                ctrl->getBitrate().get());
        delete ctrl;
        return duration;
    } else
        return computeFrameDuration(msg->getBitLength(),
                dataFrameMode->getDataMode()->getNetBitrate().get());
}
...
double Ieee80211Mac::computeFrameDuration(int bits, double bitrate) {
    ...
    if (PHY_HEADER_LENGTH < 0)
        ...
        duration = SIMTIME_DBL(modType->getDataMode()->getDuration(bits))
                + PHY_HEADER_LENGTH;

    EV_DEBUG << " duration=" << duration * 1e6 << "us(" << bits << "bits "
                    << bitrate / 1e6 << "Mbps)" << endl;
    return duration;
}

void Ieee80211Mac::logState() {
    int numCategs = numCategories();
    EV_TRACE << "# state information: mode = " << modeName(mode) << ", state = "
                    << fsm.getStateName();
    ...
    for (int i = 0; i < numCategs; i++)
        EV_TRACE << " " << i << "("
                        << (edcCAF[i].endAIFS->isScheduled() ? "scheduled" : "")
                        << ")";
    EV_TRACE << ", scheduled backoff are";
    for (int i = 0; i < numCategs; i++)
        EV_TRACE << " " << i << "("
                        << (edcCAF[i].endBackoff->isScheduled() ?
                                "scheduled" : "") << ")";
    EV_TRACE << "\n# currentAC: " << currentAC << ", oldcurrentAC: "
                    << oldcurrentAC;
    if (getCurrentTransmission() != nullptr)
        EV_TRACE << "\n# current transmission: "
                        << getCurrentTransmission()->getId();
    else
...
(    rateControlMode == RATE_CR)
    ...
    if (recovery) {
        if (retryCounter() == 1) {
            ...
            const IIeee80211Mode *slowerMode = modeSet->getSlowerMode(
                    dataFrameMode);
...
void Ieee80211Mac::setSuccessThreshold(int success_threshold) {
    if (success_threshold >= minSuccessThreshold)
        ...
        throw cRuntimeError(
                "success_threshold is less than minSuccessThreshold");
}

void Ieee80211Mac::reportRecoveryFailure(void) {
    if (rateControlMode == RATE_AARF) {
        setSuccessThreshold(
                (int) (std::min((double) getSuccessThreshold() * successCoeff,
                        (double) maxSuccessThreshold)));
        setTimerTimeout(
                (int) (std::max((double) getMinTimerTimeout(),
                        (double) (getSuccessThreshold() * timerCoeff))));
    }
}
...
double& Ieee80211Mac::throughputRecorded(int i) {
    if (i == -1)
        i = currentAC;
    if (i >= (int) edcCAF.size())
        throw cRuntimeError("AC doesn't exist");
    return edcCAF[i].throughputRecorded;
}
simtime_t& Ieee80211Mac::macDelayRecorded(int i) {
    if (i == -1)
        i = currentAC;
    if (i >= (int) edcCAF.size())
        throw cRuntimeError("AC doesn't exist");
    return edcCAF[i].macDelayRecorded;
}
simtime_t& Ieee80211Mac::jitterRecorded(int i) {
    if (i == -1)
        i = currentAC;
    if (i >= (int) edcCAF.size())
        throw cRuntimeError("AC doesn't exist");
    return edcCAF[i].jitterRecorded;
}
...
Ieee80211Mac::Ieee80211DataOrMgmtFrameList *Ieee80211Mac::transmissionQueue(
        int i) {
    if (i == -1)
        i = currentAC;
    if (i >= (int) edcCAF.size())
...
const IIeee80211Mode *Ieee80211Mac::getControlAnswerMode(
        const IIeee80211Mode *reqMode) {
    /**
    ...
    while (mode != nullptr) {
        ...
        if (modeSet->getIsMandatory(mode)
                && (!bestMode
                        || mode->getDataMode()->getGrossBitrate()
                                > bestMode->getDataMode()->getGrossBitrate())
                && mode->getDataMode()->getGrossBitrate()
                        <= reqMode->getDataMode()->getGrossBitrate() &&
...
void Ieee80211Mac::sendUp(cMessage *msg) {
    ...
    //EV << " sendUp msg->getFullName() " << msg->getFullName() << endl;
    //EV << " sendUp msg->getFullPath() " << msg->getFullPath() << endl;
...
const MACAddress& Ieee80211Mac::isInterfaceRegistered() {
    ...
    std::string interfaceName = utils::stripnonalnum(
            interfaceModule->getFullName());
    ...
    if (duplicateDetect) {    // duplicate detection filter
        ...
        if (frame) {
            ...
            if (it == asfTuplesList.end()) {
                ...
                asfTuplesList.insert(
                        std::pair<MACAddress, Ieee80211ASFTuple>(
                                frame->getTransmitterAddress(), tuple));
            } else {
                // check if duplicate
                if (it->second.sequenceNumber == frame->getSequenceNumber()
                        && it->second.fragmentNumber
                                == frame->getFragmentNumber()) {
                    return true;
                } else {
                    // actualize
...
double Ieee80211Mac::controlFrameTxTime(int bits) {
    ...
    if (PHY_HEADER_LENGTH < 0)
        ...
        duration =
        SIMTIME_DBL(controlFrameMode->getDataMode()->getDuration(bits))
                + PHY_HEADER_LENGTH;

    EV_DEBUG << " duration=" << duration * 1e6 << "us(" << bits << "bits "
                    << controlFrameMode->getDataMode()->getNetBitrate() << ")"
                    << endl;
    return duration;
}
...
void Ieee80211Mac::configureRadioMode(IRadio::RadioMode radioMode) {
    if (radio->getRadioMode() != radioMode) {
        ...
        cMessage *message = new cMessage("configureRadioMode",
                RADIO_C_CONFIGURE);
